...
@section('content')
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
            <h1 class="font-headline-lg text-3xl font-bold text-on-surface tracking-tight">Service Pages</h1>
    ...
    <div class="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm overflow-hidden">
        <div
            class="p-4 border-b border-outline-variant flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-surface-bright">
            <div class="flex items-center gap-2">
                <span
                    class="px-3 py-1.5 text-xs font-semibold rounded-md bg-secondary-container text-on-secondary-container">
                    All Pages
                    ({{ method_exists($servicePages, 'total') ? $servicePages->total() : $servicePages->count() }})
                </span>
            </div>
            <form action="{{ route('admin.service-pages.index') }}" method="GET"
                ...
                <div class="relative flex-1 sm:flex-initial">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by city..."
                        class="pl-9 pr-3 py-2 text-sm border border-outline-variant rounded-lg bg-surface-container-lowest text-on-surface placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary w-full sm:w-64 transition-all" />
                </div>
                <button type="submit"
                    class="inline-flex items-center gap-1 px-4 py-2 text-sm font-medium bg-primary text-white rounded-lg hover:shadow-md transition-all active:scale-95">
                    Search
                </button>
                @if(request('search'))
                    <a href="{{ route('admin.service-pages.index') }}"
                        class="inline-flex items-center gap-1 px-3 py-2 text-sm font-medium text-gray-600 bg-gray-100 rounded-lg hover:bg-gray-200 transition-all"
                        title="Clear search">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-outline-variant">
                    @forelse($servicePages as $service_page)
                        <tr class="hover:bg-surface-container-low/50 transition-colors group">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                {{ $service_page->created_at ? $service_page->created_at->format('M d, Y') : 'N/A' }}
                            </td>
                            ...
                            <td class="px-6 py-4 text-right whitespace-nowrap">
                                <div class="flex items-center justify-end gap-3">
                                    ...
                                    {{-- FIXED: Label string context for javascript confirm box --}}
                                    <form action="{{ route('admin.service-pages.destroy', $service_page) }}" method="POST"
                                        onsubmit="return confirm('Delete this service page permanently?');" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button
                    ...
                    @empty
                        <tr>
                            {{-- FIXED: Text context string updated --}}
                            <td colspan="4" class="px-6 py-12 text-center text-gray-400">
                                No service pages found. Click "Create New Service Page" to start adding items.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    @if(method_exists($servicePages, 'links') && $servicePages->hasPages())
        <div class="mt-6">
            {{ $servicePages->links() }}
        </div>
    @endif
