                ...
                @foreach ($provinces as $province)
                    <li>
                        <button type="button" @mouseenter="active = '{{ $province['key'] }}'"
                            :class="active === '{{ $province['key'] }}' ? 'bg-amber-500/20 text-white' : 'text-gray-300 hover:bg-white/5'"
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4">
                        @foreach ($province['cities'] as $city)
                            <a href="{{ url('services/' . $city->slug . '.html') }}"
                                class="text-gray-300 hover:text-amber-500 text-sm transition-colors whitespace-nowrap overflow-hidden text-ellipsis">
                                {{ $city->city }}
                            </a>
                        @endforeach
                    </div>
